...
/*
 * Copy of package org.apache.spark.util.collection.OpenHashSet
 *
 * Modifications:
...
class OpenHashSet[@specialized(Long, Int) T: ClassTag](
   ...
   */
  private def hashcode(h: Int): Int = Hashing.murmur3_32().hashLong(h).asInt() //Hashing.murmur3_32().newHasher().putInt(h).hash().asInt()
